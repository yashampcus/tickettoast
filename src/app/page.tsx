...
export default function Home() {
  ...
  const loadHistoryItem = (item: HistoryItem) => {
    ...
    if (item.aiAppeal) {
      localStorage.setItem('tickettoast-ai-appeal', JSON.stringify(item.aiAppeal));
    }
    if (item.formFill) {
      localStorage.setItem('tickettoast-form-fill', JSON.stringify(item.formFill));
    }
            ...
            <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        {historyItems.length} {historyItems.length === 1 ? 'item' : 'items'} saved
                      </p>
                    </div>
                  </div>
              ...
              {historyItems.length === 0 ? (
                <div className="bg-white rounded-xl shadow-lg p-12 text-center border border-gray-200">
                  ...
                  <p className="text-gray-600 mb-6">Process a document to see it appear here</p>
                  <button
                    onClick={() => setCurrentView('home')}
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        {item.result && (
                          <div className="bg-green-50 rounded-lg p-3 border-l-4 border-green-500">
                            <p className="text-xs text-gray-600 font-medium mb-1">Document Processed</p>
                            <p className="text-sm font-semibold text-gray-900">
                              {item.result.extractedFields?.length || 0} fields extracted
                            </p>
                          </div>
                        )}
                        {item.aiAppeal && (
                          <div className="bg-purple-50 rounded-lg p-3 border-l-4 border-purple-500">
                            <p className="text-xs text-gray-600 font-medium mb-1">AI Appeal Generated</p>
                            <p className="text-sm font-semibold text-gray-900">
                        ...
                        {item.formFill && (
                          <div className="bg-blue-50 rounded-lg p-3 border-l-4 border-blue-500">
                            <p className="text-xs text-gray-600 font-medium mb-1">Form Filled</p>
                            <p className="text-sm font-semibold text-gray-900">
